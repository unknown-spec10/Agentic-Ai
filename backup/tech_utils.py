...
from utils import generate_gemini_content

# Import cache manager
from cache_manager import get_cached, set_cached, delete_cached

# Import persistent database
from persistent_db import get_api_response, store_api_response, api_response_exists
...
def extract_tech_stack(description: str) -> List[str]:
    ...
    from skill_analysis import GEMINI_API_KEY, model
...
def ai_extract_tech_stack(description: str) -> List[str]:
    ...
    description_hash = description[:100]  # First 100 chars are enough for uniqueness in descriptions
    ...
    if api_response_exists('gemini_tech_stack', description_hash):
        logger.info("Using persisted tech stack extraction result")
        return get_api_response('gemini_tech_stack', description_hash)
    
    # If not in persistent DB, create the standard prompt
    prompt = f"""
    ...
    - Development tools
    - Other technical products or platforms
    
    Text:
    {description}
    
    Respond ONLY with a valid JSON array of technologies. Include only the specific technology names without explanations.
    ...
    try:
        ...
        if '[' in result and ']' in result:
            # Extract the JSON array part if there's additional text
            json_part = result[result.find('['):result.rfind(']')+1]
            tech_stack = json.loads(json_part)
            
            if isinstance(tech_stack, list):
                # Normalize tech stack names (first letter capitalized)
                normalized_stack = [tech.strip() for tech in tech_stack if tech.strip()]
                normalized_stack = sorted(list(set(normalized_stack)))
                
                # Store the normalized result in persistent database for future deterministic outputs
                store_api_response('gemini_tech_stack', description_hash, normalized_stack, {
                ...
                logging.warning("Unexpected JSON structure in Gemini response for tech stack")
        else:
            logging.warning("AI response does not contain valid JSON array for tech stack")
    except Exception as e:
        logging.error(f"Error in AI tech stack extraction: {e}")
    
    # Fall back to rule-based extraction if AI extraction fails
    return rule_based_extract_tech_stack(description)
