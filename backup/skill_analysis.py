...
def ai_extract_job_requirements(job_description: str) -> List[str]:
    ...
    For each skill, identify if it is required/mandatory or preferred/nice-to-have.
    
    Job Description:
    {job_description}
    
    Respond ONLY with a valid JSON object containing the required skills. Do not include any explanations or text outside of the JSON object.
    ...
    try:
        ...
        if '{' in result and '}' in result:
            ...
            if 'required_skills' in result_dict and isinstance(result_dict['required_skills'], list):
                return sorted(list(set(result_dict['required_skills'])))
            else:
                logging.warning("Unexpected JSON structure in Gemini response for job requirements")
        else:
            logging.warning("AI response does not contain valid JSON for job requirements")
    except Exception as e:
        logging.error(f"Error in AI job requirements extraction: {e}")
    
    # Fall back to empty list if extraction fails
    return []


def rule_based_extract_job_requirements(job_description: str) -> List[str]:
    """Extract required skills from job description using rule-based approach."""
    # Simple keyword extraction for most common skills
    common_technologies = [
    ...
    for skill in common_technologies:
        skill_lower = skill.lower()
        # Define patterns that indicate required skills
        required_patterns = [
            ...
            f"expertise in.*?{skill_lower}",
            f"skills:.*?{skill_lower}",
            f"requirements:.*?{skill_lower}"
        ]
...
def recommend_job_roles(skills: List[str], location: str = "india", experience_years: float = 0) -> Dict[str, Any]:
    ...
    try:
        # Get Adzuna credentials from Streamlit secrets instead of environment variables
